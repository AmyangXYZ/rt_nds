...
impl Packet {
    ...
    pub fn from_buffer(buffer: &[u8]) -> Self {
        ...
        packet.magic = u16::from_le_bytes(buffer[0..2].try_into().unwrap());
        packet.uid = u64::from_le_bytes(buffer[2..10].try_into().unwrap());
        packet.ptype = buffer[10];
        packet.token = u64::from_le_bytes(buffer[11..19].try_into().unwrap());
        packet.priority = buffer[19];
        packet.data_name = String::from_utf8_lossy(&buffer[20..84]).to_string();
        packet.data_size = u32::from_le_bytes(buffer[84..88].try_into().unwrap());
        packet.data_chunk_num = u16::from_le_bytes(buffer[88..90].try_into().unwrap());
        packet.data_chunk_id = u16::from_le_bytes(buffer[90..92].try_into().unwrap());
        packet.data_checksum = u32::from_le_bytes(buffer[92..96].try_into().unwrap());
        packet.data = buffer[96..].to_vec();
        packet
    }
